...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("Abaddon was updated to the latest version")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("Abaddon will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['nordbearbot.dev', 'pavelstar', 'egor game' 'Ilya Lipnickiy ', 'Matwei Yaskelyainen']
...
description = """Abaddon - Your Demonic Spammer Application

Abaddon can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Demonic Spamming
\t Testing Your Spam Detector and more ....

Abaddon is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start Abaddon with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start Abaddon with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start Abaddon with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update Abaddon")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current Abaddon contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current Abaddon version")
